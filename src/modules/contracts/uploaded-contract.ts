...
export async function prepareUploadedContract(file?: { buffer: Buffer; mimetype: string }) {
  if (!file || file.mimetype !== 'application/pdf' || file.buffer.length > 10 * 1024 * 1024 ||
    ...
    throw new BadRequestException('Upload a PDF file (maximum 10 MB).')
  }
  try {
    ...
    throw new BadRequestException('The PDF cannot be read. Upload an unencrypted PDF with 1–100 pages.')
  }
}
